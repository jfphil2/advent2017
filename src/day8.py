...
TEST_OUTPUT = {'b': 0, 'a': 1, 'c': -10}

TEST_HIGHEST_VALUE = ('a', 1)

# Python doesn't support case statements, dictionary lookup keeps things pretty
OPERATOR_LOOKUP = {
    'inc': operator.add,
    'dec': operator.sub,
    '<'  : operator.lt,
...
def register_puzzle(input):
    ...
    pattern = re.compile("""
            ...
            (?P<test_value>-?\d+)$           # value for conditional
    """, re.VERBOSE)
    ...
    for line in input.strip('\n').split('\n'):
        ...
        # Add new registers to the list of known registers
        if instruction['register'] not in registers.keys():
            registers[instruction['register']] = 0

        if instruction['test_register'] not in registers.keys():
            registers[instruction['test_register']] = 0

        # Evaluate the conditional
        conditional_func = OPERATOR_LOOKUP[instruction['test_operator']]
        condition = conditional_func(registers[instruction['test_register']],
                                     int(instruction['test_value']))

        if condition == True:
            # Modify the registers
            modification_func = OPERATOR_LOOKUP[instruction['operation']]
...
def get_maximum_register(registers):
    return max(registers.items(), key=operator.itemgetter(1))


if __name__ == '__main__':
    test_registers = register_puzzle(TEST_INPUT)
    assert test_registers.keys() == TEST_OUTPUT.keys()
    assert list(test_registers.values()) == list(TEST_OUTPUT.values())
    assert get_maximum_register(test_registers) == TEST_HIGHEST_VALUE

    registers = register_puzzle(INPUT)
    print('Day 8: Part 1:', get_maximum_register(registers))